...
router.get("/classes/:classId/generate-teams", isLoggedIn, isInstructor, async (req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: "An error occurred while accessing this page." });
  }
});

// Handle team generation for a specific class
router.post("/classes/:classId/generate-teams", isLoggedIn, isInstructor, upload.single("csvFile"), async (req, res) => {
  try {
    const { classId } = req.params;
